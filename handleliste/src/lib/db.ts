...
export async function fetchList(): Promise<ShoppingItem[]> {
  const { data, error } = await supabase.from(LIST).select('*').order('created_at');
  fail('Klarte ikke å hente handlelista', error);
  return (data ?? []) as ShoppingItem[];
}

export async function fetchWeekPlan(): Promise<WeekPlanItem[]> {
  const { data, error } = await supabase.from('week_plan_items').select('id,meal_id,added_to_list');
...
async function applyPending(pending: readonly PendingItem[]): Promise<void> {
  ...
  if (inserts.length > 0) {
    const { error } = await supabase.from(LIST).insert(inserts.map(insertPayload));
    ...
    if (error !== null && error.code === '23505') {
      ...
      if (retry.inserts.length > 0) {
        const { error: insertError } = await supabase.from(LIST).insert(retry.inserts.map(insertPayload));
...
export async function setChecked(id: string, checked: boolean): Promise<void> {
  const { error } = await supabase.from(LIST).update({ checked }).eq('id', id);
...
export async function removeCheckedItems(): Promise<void> {
  const { error } = await supabase.from(LIST).delete().eq('checked', true);
  fail('Klarte ikke å fjerne avhukede varer', error);
}

export async function clearList(): Promise<void> {
  const { error } = await supabase.from(LIST).delete().not('id', 'is', null);
...
export async function addMealToWeek(mealId: string): Promise<void> {
  const { error } = await supabase.from('week_plan_items').insert({ meal_id: mealId });
...
export async function removeMealFromWeek(mealId: string): Promise<void> {
  const { error } = await supabase.from('week_plan_items').delete().eq('meal_id', mealId);
...
export async function clearWeek(): Promise<void> {
  const { error } = await supabase.from('week_plan_items').delete().not('id', 'is', null);
